...
function calendar(y, m) {
    ...
    if (m == 3) this[tmp1].color = '#78909c'; //清明颜色
    ...
    for (i  in  sFtv)
        if (sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
            if (Number(RegExp.$1) == (m + 1)) {
                ...
                if (RegExp.$3 == '*')  this[Number(RegExp.$2) - 1].color = '#78909c'
            }
        ...
        if (lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
            ...
            if (tmp1 >= 0 && tmp1 < n) {
                ...
                if (tmp2 >= 0 && tmp2 < this.length) {
                    ...
                    if (RegExp.$3 == '*')  this[tmp2].color = '#78909c'
                }
            }
        }
...
function drawCld(SY, SM) {
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            if (cld[sD].solarTerms == '清明') s = '清明节'.fontcolor('#78909c');
            if (cld[sD].solarTerms == '芒种') s = '芒种'.fontcolor('#78909c');
            if (cld[sD].solarTerms == '夏至') s = '夏至'.fontcolor('#78909c');
            if (cld[sD].solarTerms == '冬至') s = '冬至'.fontcolor('#78909c');
...
function mOvr(thisObj, v) {
    ...
    if (sObj.html() != '') {
        ...
        arr.push('<div id="teshu"><FONT  COLOR="#78909c"  STYLE="font-size:12px;">' + cld[d].solarTerms + ' ' + cld[d].solarFestival + ' ' + cld[d].lunarFestival + '</FONT></div>');
        ...
        arr.push('<font style="font-size:12px;line-height:28px"><b>' + '星期' + cld[d].week + '</b></font>');
...
function mOck(thisObj, v){
	...
	if(onoff == '0'){
		...
		hDays.push(dayJson);            
                var clad=thisObj.id.substr(2);
                var liDay="<li id=myDay"+day+"><a href='javascript:;'onClick='clearMyDay("+day+","+clad+")'  id=closeDay"+day+"><font>"+dayF+"</font><i><img src=assets/img/icon-remove.png  > 取消预定</i> </a></li>";
                $("#ulDay").append(liDay);
